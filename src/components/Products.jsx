...
export function ProductCard({ product, delay = 0 }) {
  ...
  return (
    <article
      ref={ref}
      className="group flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-red-200 hover:shadow-xl"
      style={{
        opacity: inView ? 1 : 0,
        transform: inView ? "translateY(0)" : "translateY(24px)",
        transition: `opacity 0.55s ease-out ${delay}ms, transform 0.55s ease-out ${delay}ms, box-shadow 0.3s, border-color 0.3s`,
      }}
    >
      {/* PRODUCT CONTENT */}
      <div>
        ...
        <div className="relative h-60 w-full overflow-hidden bg-gray-100">
          ...
          {/* Yellow Accent */}
          <span
            className="absolute bottom-4 left-4 h-1 w-12 rounded-full"
            style={{
        ...
        <div className="p-6 pb-0">
          ...
          <ul className="mt-5 flex list-none flex-wrap gap-2 p-0">
            {product.specs.map((spec) => (
              <li
                key={spec}
                className="inline-flex items-center gap-1.5 rounded-md bg-gray-50 px-2.5 py-1.5 text-[11px] font-semibold text-gray-700"
              >
                <Check
      ...
      <div className="mt-6 flex items-center justify-between border-t border-gray-100 px-6 py-4">
        ...
        <a
          href={product.href}
          className="inline-flex items-center gap-1.5 rounded-lg px-4 py-2.5 text-xs font-bold text-white transition-all duration-200 hover:shadow-lg active:scale-[0.97]"
          style={{
      ...
      <div className="mx-auto max-w-7xl">
        ...
        >
          ...
          <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-gray-600">
            Discover our quality food products, carefully
            developed with a focus on freshness, taste,
            ...
            {categories.map((category) => {
              ...
              return (
                <button
                  ...
                  className="rounded-full border px-4 py-2 text-xs font-semibold transition-all duration-200 hover:scale-[1.03] active:scale-[0.97]"
                  style={{
